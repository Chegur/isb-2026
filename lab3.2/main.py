...
def run_generation():
    """
    Запускает процесс генерации ключей гибридной системы.
    
    Пользователь выбирает пути для сохранения:
    - Зашифрованного симметричного ключа.
    - Публичного RSA-ключа.
    - Приватного RSA-ключа.
    Также указывается длина симметричного ключа в байтах.
    
    Процесс включает:
    1. Генерацию случайного симметричного ключа.
    2. Генерацию пары ключей RSA.
    3. Шифрование симметричного ключа публичным ключом RSA.
    4. Сериализацию и сохранение всех ключей в файлы.
    """
...
def run_encryption():
    """
    Запускает процесс шифрования файла гибридной системой.
    
    Требует наличия:
    - Исходного файла.
    - Файла приватного ключа (для расшифровки сеансового ключа).
    - Файла зашифрованного симметричного ключа.
    
    Алгоритм:
    1. Загрузка приватного ключа и зашифрованного симметричного ключа.
    2. Расшифровка симметричного ключа (получение доступа к сеансовому ключу).
    3. Чтение исходного файла.
    4. Применение паддинга и шифрование данных алгоритмом Camellia.
    5. Сохранение результата (IV + Шифротекст) в новый файл.
    """
...
def run_decryption():
    """
    Запускает процесс дешифрования файла гибридной системой.
    
    Требует наличия:
    - Зашифрованного файла (содержащего IV и шифротекст).
    - Файла приватного ключа.
    - Файла зашифрованного симметричного ключа.
    
    Алгоритм:
    1. Загрузка приватного ключа и зашифрованного симметричного ключа.
    2. Расшифровка симметричного ключа.
    3. Чтение зашифрованного файла и разделение на IV и шифротекст.
    4. Дешифрование данных алгоритмом Camellia (включает автоматическое удаление паддинга).
    5. Сохранение восстановленного исходного файла.
    """
...
def main():
    """
    Главная функция программы, реализующая циклическое консольное меню.
    
    Поддерживает ввод команд в различных форматах (цифры, буквы, слова на РУС/ENG).
    Использует конструкцию match-case для маршрутизации действий.
    """
    commands = {
